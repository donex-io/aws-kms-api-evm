...
class KmsApiStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, eth_network: str = 'rinkeby', **kwargs) -> None:
        ...
        cmk.grant(kms_lambda.lf, 'kms:GetPublicKey')
        cmk.grant(kms_lambda.lf, 'kms:Sign')

        kms_api = apigateway.LambdaRestApi(self, "kms-api-ethereum",
                    handler=kms_lambda.lf,
                    rest_api_name='kms-api-ethereum'
        )

        items = kms_api.root.add_resource('items')   
